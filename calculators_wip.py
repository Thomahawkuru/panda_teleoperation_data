...
def grabs(data, p, c, t):
    ...
    dtime = np.mean(data[p][c][t]['Gripper']["dt"].loc[data[p][c][t]['Experiment'].start].reset_index(drop = True))

    peaks_succes, _ = signal.find_peaks(grab_crop, height = [-0.035, -0.015], prominence=0.005, distance=1/dtime)
    peaks_fail, _   = signal.find_peaks(grab_crop, height = -0.02, prominence=0.005, distance=1/dtime)
...
def grab_velocity(data, p, c, t, file, pre_time):
    ...
    peaks_succes, _ = signal.find_peaks(grab_crop, height = [-0.035, -0.015], prominence=0.005, distance=1/np.mean(input_data['dt']))
    ...
    # grab_stop = input_crop.iloc[endpoints,[1,2,3]]
    ...
    # fig3 = px.scatter_3d(grab_stop, x='posZ', y='posX', z='posY')
